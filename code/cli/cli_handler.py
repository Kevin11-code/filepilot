...
from ..utils.secure_string import SecureString, get_credential_manager, SecureTemporaryCredentials
...
class CLIHandler:
    ...
    def parse_args(self):
        ...
        s2s_parser.add_argument('--source-host', help='Source server hostname or IP')
        s2s_parser.add_argument('--source-username', help='Source server username')
        s2s_parser.add_argument('--source-password', help='Source server password')
        s2s_parser.add_argument('--dest-host', help='Destination server hostname or IP')
        s2s_parser.add_argument('--dest-username', help='Destination server username')
        s2s_parser.add_argument('--dest-password', help='Destination server password')
        s2s_parser.add_argument('--source-connection', type=str, help='Use a saved source connection')
        s2s_parser.add_argument('--dest-connection', type=str, help='Use a saved destination connection')
    ...
    def _handle_upload_secure(self, args):
        ...
        with SecureTemporaryCredentials(connect_params) as temp_params:
            ...
            if not client.connect(**temp_params):
                ...
                sys.exit(1)
        # Credentials automatically cleared here
    ...
    def _handle_server_to_server_secure(self, args):
        ...
        # Prepare connection parameters for both configs
        ...
        # Use nested secure context managers for both credential sets
        with SecureTemporaryCredentials(source_connect_config) as source_temp_params:
            with SecureTemporaryCredentials(dest_connect_config) as dest_temp_params:
        ...
        # Both credential sets automatically cleared here
